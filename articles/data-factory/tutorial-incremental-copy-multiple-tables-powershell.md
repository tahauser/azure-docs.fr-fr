...
> * Ajouter ou mettre à jour des données dans des tables source
...
3. **Créer un pipeline avec les activités suivantes :** 
    ...
    1. Créer une activité **ForEach** qui effectue une itération dans une liste de noms de table source qui est transmise en tant que paramètre au pipeline. Pour chaque table source, elle appelle les activités suivantes pour effectuer le chargement delta pour cette table. 
    2. Créez deux activités de **recherche**. Servez-vous de la première activité de recherche pour récupérer la dernière valeur de limite. Servez-vous de la deuxième activité de recherche pour récupérer la nouvelle valeur de limite. Ces valeurs de limite sont passées à l’activité de copie. 
    3. Créez une **activité de copie** qui copie les lignes du magasin de données sources dont la valeur de la colonne est supérieure à l’ancienne valeur et inférieure à la nouvelle. Elle copie ensuite les données delta du magasin de données sources dans un stockage Blob sous la forme d’un nouveau fichier. 
    4. Créez une **activité de procédure stockée** qui met à jour la valeur de limite pour le pipeline s’exécutant la prochaine fois. 

        Voici le schéma de la solution générale : 
...
* **Base de données SQL Azure**. Vous allez utiliser une base de données SQL Azure comme magasin de données **sink**. Si vous n’avez pas de base de données Azure SQL Database, consultez l’article [Création d’une base de données Azure SQL](../sql-database/sql-database-get-started-portal.md) pour savoir comme en créer une. 

### <a name="create-source-tables-in-your-sql-server-database"></a>Créer des tables sources dans votre base de données SQL Server

1. Lancez **SQL Server Management Studio** et connectez-vous à votre serveur SQL local. 
...
2. Dans l’**Explorateur de serveurs**, cliquez avec le bouton droit sur la **base de données** et choisissez **Nouvelle requête**.
3. Exécutez la commande SQL suivante sur votre base de données Azure SQL pour créer des tables nommées `customer_table` et `project_table`.  
...
### <a name="create-another-table-in-azure-sql-database-to-store-the-high-watermark-value"></a>Créer une autre table dans la base de données Azure SQL pour stocker la valeur de limite supérieure
1. Exécutez la commande SQL suivante sur votre base de données Azure SQL Database pour créer une table sous le nom `watermarktable` pour stocker la valeur de limite.  
    ...
    ```
3. Insérer des valeurs de limite supérieure initiale pour les deux tables source dans la table de limite supérieure.
...
Exécutez la commande suivante pour créer une procédure stockée dans votre base de données Azure SQL Database. Cette procédure stockée met à jour la valeur de la limite supérieure après chaque exécution du pipeline. 
...
Créez deux procédures stockées et deux types de données dans votre base de données Azure SQL en exécutant la requête suivante : celles-ci sont utilisées pour fusionner les données des tables sources dans les tables de destination.
...
3. Définissez une variable pour le nom de la fabrique de données. 
    ...
    > [!IMPORTANT]
    >  Mettez à jour le nom de la fabrique de données afin qu’il soit globalement unique. Par exemple, ADFIncMultiCopyTutorialFactorySP1127. 
...
Vous allez créer des services liés dans une fabrique de données pour lier vos magasins de données et vos services de calcul à la fabrique de données. Dans cette section, vous créez des services liés à vos bases de données Azure SQL et SQL Server locales. 

### <a name="create-sql-server-linked-service"></a>Créer le service lié SQL Server.
Dans cette étape, vous liez votre serveur SQL Server local à la fabrique de données.

1. Créez un fichier JSON nommé **SqlServerLinkedService.json** dans le dossier **C:\ADFTutorials\IncCopyMultiTableTutorial** avec le contenu suivant : sélectionnez la section appropriée en fonction de **l’authentification** utilisée pour établir la connexion à SQL Server. Créer les dossiers locaux s’ils n’existent pas encore. 
...
2. Dans **Azure PowerShell**, basculez vers le dossier **C:\ADFTutorials\IncCopyMultiTableTutorial**.
...
Dans cette étape, vous créez des jeux de données pour représenter la source de données et la destination de données. et l’emplacement pour stocker la limite supérieure.
...
1. Créez un fichier JSON sous le nom **SourceDataset.json** dans le même dossier avec le contenu suivant : 
    ...
    Le nom de table est factice. L’activité de copie dans le pipeline utilise une requête SQL pour charger les données au lieu de charger l’ensemble de la table. 
1.  Exécutez l’applet de commande Set-AzureRmDataFactoryV2Dataset pour créer le jeu de données SourceDataset.
...
1. Créez un fichier JSON nommé **SinkDataset.json** dans le même dossier avec le contenu suivant : le paramètre tableName est défini par le pipeline de manière dynamique lors de l’exécution. L’activité ForEach du pipeline effectue une itération dans une liste de noms de table et transmet le nom de table à ce jeu de données dans chaque itération. 
...
Ce pipeline prend une liste de noms de tables comme paramètre. L’**activité ForEach** effectue une itération au sein de la liste de noms de tables et effectue les opérations suivantes : 

1. Utilisez l’**activité de recherche** pour récupérer l’ancienne valeur de la limite supérieure (valeur initiale ou celle utilisée dans la dernière itération).
2. Utilisez l’**activité de recherche** pour récupérer la nouvelle valeur de la limite supérieure (valeur maximale de la colonne de limite supérieure de la table source).
3. Utilisez l’**activité de copie** pour copier des données entre ces deux valeurs de limite supérieure depuis la base de données source vers la base de données de destination. 
4. Utilisez l’**activité de procédure stockée** pour mettre à jour l’ancienne valeur de limite supérieure à utiliser au cours de la première étape de l’itération suivante. 
...
1. Créez un fichier de paramètres sous le nom **Parameters.json** dans le même dossier avec le contenu suivant :
    ...
    ```
2. Exécutez le pipeline **IncrementalCopyPipeline** en utilisant l’applet de commande **Invoke-AzureRmDataFactoryV2Pipeline**. Remplacez les espaces réservés par votre propre groupe de ressources et votre propre nom de fabrique de données.
...
6. Lorsque vous cliquez sur le lien dans la colonne **Actions**, la page suivante affiche toutes les **exécutions d’activité** du pipeline. 
    ...
    ![Exécutions d’activités](media\tutorial-incremental-copy-multiple-tables-powershell\monitor-activity-runs-5.png)
...
Dans SQL Server Management Studio, exécutez les requêtes suivantes sur la base de données Azure SQL pour vérifier que les données ont été copiées à partir des tables sources vers les tables de destination. 
...
Notez que les valeurs de limite supérieure des deux tables ont été mises à jour. 

## <a name="add-more-data-to-the-source-tables"></a>Ajouter plus de données aux tables sources

Exécutez la requête suivante sur la base de données SQL Server source pour mettre à jour une ligne existante dans customer_table et insérez une nouvelle ligne dans project_table. 
...
1. Maintenant, exécutez à nouveau le pipeline à l’aide de la commande PowerShell suivante :
    ...
    $RunId = Invoke-AzureRmDataFactoryV2Pipeline -PipelineName "IncrementalCopyPipeline" -ResourceGroup $resourceGroupname -dataFactoryName $dataFactoryName -ParameterFile ".\Parameters.json"
    ```
2. Analysez les exécutions du pipeline en suivant les instructions de la section [Surveiller le pipeline](#monitor-the-pipeline). Dans la mesure où l’état du pipeline est **En cours**, un autre lien d’action s’affiche sous **Actions** pour annuler l’exécution du pipeline. 

    ![Exécutions de pipeline](media\tutorial-incremental-copy-multiple-tables-powershell\monitor-pipeline-runs-6.png)    
3. Cliquez sur **Actualiser** pour actualiser la liste jusqu’à ce que l’exécution du pipeline réussisse. 

    ![Exécutions de pipeline](media\tutorial-incremental-copy-multiple-tables-powershell\monitor-pipeline-runs-succeded-7.png)
4. (facultatif) Cliquez sur le lien **Afficher les exécutions du pipeline** (icône) sous Actions pour voir toutes les exécutions de l’activité associée à l’exécution de ce pipeline. 

## <a name="review-final-results"></a>Passer en revue les résultats finaux
...
Notez les nouvelles valeurs de Name et LastModifytime pour le PersonID: 3. 
