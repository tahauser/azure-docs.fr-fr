...
---
# <a name="cost-estimation-report-of-azure-site-recovery-deployment-planner"></a>Rapport d’estimation de coût d’Azure Site Recovery Deployment Planner  
...
**Coût par état** Le coût de la récupération d’urgence total (DR) est catégorisé selon deux états différents, la réplication et la récupération d’urgence. 
...
Le coût total de la récupération d’urgence (DR) est catégorisé selon deux états différents, la réplication et la récupération d’urgence.

**Coût de la réplication** : le coût induit au moment de la réplication. Il couvre le coût du stockage, du réseau et de la licence d’Azure Site Recovery. 

**Coût d’extraction de la récupération d’urgence** : le coût induit au moment de l’extraction de la récupération d’urgence. Azure Site Recovery prépare des machines virtuelles pendant les extractions de la récupération d’urgence. Le coût d’extraction de la récupération d’urgence couvre les coûts de calcul et de stockage des machines virtuelles en cours d’exécution.
Extraction de la récupération d’urgence dans une année = nombre d’extractions de la récupération d’urgence x chaque extraction de la récupération d’urgence (jours) Coût moyen de l’extraction de la récupération d’urgence (par mois) = Coût total de l’extraction de la récupération d’urgence / 12
...
Si vous disposez de n’importe quel contrôleur de domaine ou machine virtuelle DNS ou SQL Server dotée de groupes de disponibilité AlwaysOn s’exécutant sur Azure pour la récupération d’urgence, vous pouvez indiquer le nombre de machines virtuelles et la taille afin de prendre en compte le calcul de leur coût dans le coût de la récupération d’urgence. 
...
## <a name="detailed-cost-analysis-table"></a>Table d’analyse des coûts détaillée
...
3.  Vous pouvez appliquer la même valeur à toutes les machines virtuelles du tableau en cliquant sur le bouton « Appliquer à tous » pour le nombre d’extraction de la récupération d’urgence par an, la durée de chaque extraction de la récupération d’urgence (en jours), la redondance des données et les avantages d’Azure hybride.

4.  Cliquez sur « Recalculer le coût » pour mettre à jour le coût.

**VMName** : nom de la machine virtuelle.

**Nombre de machines virtuelles** : nombre de machines virtuelles qui correspondent à la configuration. Vous pouvez mettre à jour le nombre de machines virtuelles existantes si des machines virtuelles de configuration similaire ne sont pas profilées mais sont protégées.
...
**Type de système d’exploitation** : type de système d’exploitation de la machine virtuelle. Il peut s’agir de Windows ou de Linux. Si le type de système d’exploitation est Windows, l’avantage Azure hybride peut être appliqué à cette machine virtuelle. 
...
En savoir plus sur la protection des [machines virtuelles de VMware vers Azure à l’aide d’Azure Site Recovery](https://docs.microsoft.com/en-us/azure/site-recovery/tutorial-vmware-to-azure).