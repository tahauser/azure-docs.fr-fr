...
Si vous ne parvenez pas à sélectionner votre machine virtuelle Azure quand vous activez la réplication, c’est peut-être parce que la configuration de Site Recovery laissée sur celle-ci est obsolète. Une configuration obsolète peut être conservée sur une machine virtuelle Azure dans les cas suivants :
