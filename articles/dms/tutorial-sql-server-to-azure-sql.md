...
> * Créer un projet de migration Azure Database Migration Service.
...
- Les informations d’identification utilisées pour se connecter à l’instance Azure SQL Database cible doivent disposer des autorisations de CONTRÔLE DE BASE DE DONNÉES sur les bases de données Azure SQL Database cibles.
- Votre pare-feu Windows doit être ouvert pour permettre à Azure Database Migration Service d’accéder au SQL Server source.

## <a name="assess-your-on-premises-database"></a>Évaluer votre base de données locale
Avant de pouvoir migrer des données d’une instance SQL Server locale vers Azure SQL Database, vous devez évaluer les problèmes de blocage dans la base de données SQL Server qui peuvent empêcher la migration. Après avoir téléchargé et installé l’Assistant Migration des données version 3.3, suivez les étapes décrites dans l’article [Évaluation de la migration de SQL Server](https://docs.microsoft.com/sql/dma/dma-assesssqlonprem) pour évaluer la base de données locale. Un résumé de la procédure est présenté ci-dessous :
...
6.  Sélectionnez **AdventureWorks2012**, **Ajouter**, puis **Démarrer l’évaluation**.
...
7.  Passez en revue les résultats d’évaluation en ce qui concerne les problèmes de blocage de migration et les problèmes de parité de fonctionnalité en sélectionnant les options spécifiques.
    ...
    - La catégorie de problèmes de compatibilité indique les fonctionnalités partiellement ou non prises en charge reflétant les éventuels problèmes de compatibilité susceptibles de bloquer la migration de la ou des bases de données SQL Server locales vers une ou des Azure SQL Database. Des recommandations sont également fournies pour vous aider à résoudre ces problèmes.


## <a name="migrate-the-sample-schema"></a>Migrer l’exemple de schéma
Une fois que vous êtes à l’aise avec l’évaluation et que la base de données sélectionnée est un bon candidat pour la migration vers Azure SQL Database, utilisez l’Assistant Migration des données pour migrer le schéma vers Azure SQL Database.

> [!NOTE]
> Avant de créer un projet de migration dans l’Assistant Migration des données, vérifiez que vous avez bien approvisionné une base de données SQL Azure comme indiqué dans les conditions préalables. Dans ce didacticiel, le nom d’Azure SQL Database est censé être **AdventureWorks2012**, mais vous pouvez la nommer différemment si vous le souhaitez.

Pour migrer le schéma **AdventureWorks2012** vers Azure SQL Database, procédez comme suit :

1.  Lancez l’Assistant Migration des données.
2.  Sélectionnez l’icône Nouveau (+) puis, sous **Type de projet**, sélectionnez **Migration**.
...
7.  Sélectionnez **Suivant**, sous **Se connecter au serveur cible**, spécifiez les détails de connexion cible de la base de données Azure SQL, sélectionnez **Se connecter**, puis sélectionnez la base de données **AdventureWorks2012** que vous avez pré-approvisionnée dans la base de données Azure SQL.
...
1.  Connectez-vous au portail Azure, sélectionnez **+ Créer une ressource**, recherchez Azure Database Migration Service, puis sélectionnez **Azure Database Migration Service** dans la liste déroulante.
