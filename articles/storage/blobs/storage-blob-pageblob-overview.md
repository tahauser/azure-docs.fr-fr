...
Les principales fonctionnalités des objets blob de pages Azure résident dans son interface REST, la durabilité du stockage sous-jacent et les fonctionnalités de migration parfaite vers Azure. Nous allons décrire ces fonctionnalités plus en détails dans la section suivante. De plus, les objets blob de pages Azure sont actuellement pris en charge sur deux types de stockage : Stockage Premium et Standard. Stockage Premium est plus particulièrement conçu pour les charges de travail nécessitant de hautes performances et une faible latence, ce qui rend les objets blob de pages premium idéaux pour les bases de données de stockage de données hautes performances.  Le stockage Standard est plus économique pour l’exécution de charges de travail insensibles à la latence.
...
* Gestion des captures instantanées incrémentielle orientée application : les applications peuvent exploiter les instantanés d’objet blob de pages et les API REST pour enregistrer les points de contrôle d’application sans duplication coûteuse de données. Ceci est possible car nous prenons en charge les instantanés locaux pour les objets blob de pages, qui ne nécessitent pas une copie de l’ensemble des données. Ces API d’instantané publiques permettent également d’accéder à et de copier des deltas entre deux instantanés.
* Migration dynamique d’application et de données locales vers le cloud : copiez les données locales et utilisez les API REST pour écrire directement dans l’objet blob de pages Azure pendant l’exécution de la machine virtuelle locale. Lorsque la cible est interceptée, vous pouvez basculer rapidement vers la machine virtuelle Azure à l’aide de ces données. Cela permet de migrer vos machines virtuelles et disques virtuels localement vers le cloud avec un temps d’arrêt minime car la migration des données se produit en arrière-plan alors que vous continuez à utiliser la machine virtuelle. Le temps d’arrêt nécessaire pour le basculement sera donc court (quelques minutes).
...
Consultez le document suivant pour commencer à [développer à l’aide d’objets blob de pages](storage-dotnet-how-to-use-blobs.md). Par exemple, découvrons comment accéder aux objets blob de pages à l’aide de la bibliothèque cliente de stockage pour .NET. 
...
Pour écrire des pages, utilisez la méthode [CloudPageBlob.WritePages](/library/microsoft.windowsazure.storageclient.cloudpageblob.writepages.aspx).  Cela vous permet d’écrire un ensemble séquentiel de pages jusqu’à 4 Mo. Le décalage écrit doit commencer sur une limite de 512 octets (startingOffset % 512 == 0) et se terminer sur une limite de 512 - 1.  Le code suivant montre un exemple d’appel de WritePages pour un objet blob auquel nous accédons :
```csharp
...
L’API REST des objets blob de pages et son mécanisme de leasing permet aux applications d’accéder à l’objet blob de pages à partir de plusieurs clients. Par exemple, supposons que vous devez créer un service cloud distribué qui partage des objets de stockage avec plusieurs utilisateurs. Il pourrait s’agir d’une application web proposant une collection d’images importante à plusieurs utilisateurs. Une option d’implémentation consiste à utiliser une machine virtuelle avec des disques attachés. Ceci implique des inconvénients, notamment (i) la contrainte qu’un disque ne peut être attaché qu’à une seule machine virtuelle, limitant ainsi l’évolutivité, la flexibilité et augmentant les risques. En cas de problème lié à la machine virtuelle ou au service exécuté sur la machine virtuelle, et ensuite dû au bail, l’image sera inaccessible jusqu’à ce que le bail expire ou qu’il soit interrompu ; et (ii) le coût supplémentaire lié à une machine virtuelle IaaS. 

Une autre option consiste à utiliser les objets blob de pages directement via les API REST de Stockage Azure. Cette option supprime la nécessite de machines virtuelles IaaS coûteuses, offre la flexibilité maximale d’accès direct à partir de plusieurs clients, simplifie la gestion des services en supprimant la nécessité d’attacher/de détacher des disques, et élimine le risque de problèmes sur la machine virtuelle. Elle offre également le même niveau de performances pour les opérations de lecture/écriture aléatoires qu’un disque
...
Azure s’efforce d’offrir la meilleure expérience à nos clients. Nous développons la plateforme de stockage avec une durabilité des données de classe professionnelle, nos clients peuvent donc faire confiance à Azure pour leurs données critiques. Azure offre une expérience de prise en charge et de développement d’API unique pour les objets blob de pages qu’aucune autre plateforme de cloud public n’est capable de proposer. Cela a conduit à nombreuses innovations internes et tierces comme des migrations parfaites vers le cloud, une expérience de sauvegarde/récupération d’urgence supérieure, la prise en charge de PaaS et DBaaS, des solutions de stockage distribué et d’autres innovations pour les machines virtuelles/disques IaaS. Plus généralement, Azure surpasse toutes les plateformes de cloud public grâce à ces fonctionnalités uniques, et les clients Azure tirent profit des valeurs ajoutées qu’aucune autre plateforme de cloud ne peut fournir.
