
# [Documentation Batch](index.yml)

# Vue d'ensemble
## [Présentation d’Azure Batch](batch-technical-overview.md)

# Démarrages rapides
## [Exécuter un travail Batch - CLI](quick-create-cli.md)
## [Exécuter un travail Batch - Portail](quick-create-portal.md)
## [Exécuter un travail Batch - .NET](quick-run-dotnet.md)
## [Exécuter un travail Batch - Python](quick-run-python.md)


# Didacticiels

## [Traitement parallèle de fichiers - .NET](tutorial-parallel-dotnet.md)
## [Traitement parallèle de fichiers - Python](tutorial-parallel-python.md)
## [Rendu de scènes avec Arnold](tutorial-rendering-cli.md)
## [Simulation parallèle R](tutorial-r-doazureparallel.md)

# Exemples
## Exemples de code
### [Exemples de code Azure](https://azure.microsoft.com/resources/samples/?service=batch)
### [Référentiel d’exemples Batch](https://github.com/Azure/azure-batch-samples)
## [interface de ligne de commande Azure](cli-samples.md)

# Concepts
## [Fonctionnalités pour développeur](batch-api-basics.md)
## [API et outils](batch-apis-tools.md)
## [Quotas et limites](batch-quota-limit.md)

# Procédures
## Gestion des comptes Batch
### [Gérer des comptes Batch avec le portail](batch-account-create-portal.md)
### [Gérer les comptes Batch avec Batch Management .NET](batch-management-dotnet.md)

## S’authentifier avec Azure AD
### [Azure AD avec service Batch](batch-aad-auth.md)
### [Azure AD avec gestion de lots](batch-aad-auth-management.md)

## [Créer des packages d’applications](batch-application-packages.md)

## Créer et gérer des pools
### [Mettre automatiquement à l’échelle les nœuds de calcul](batch-automatic-scaling.md)
### [Choisir une taille de VM pour les nœuds de calcul](batch-pool-vm-sizes.md)
### [Configurer l’accès aux nœuds de calcul](pool-endpoint-configuration.md)
### [Créer un pool avec une image personnalisée](batch-custom-images.md)
### [Créer un pool dans un réseau virtuel](batch-virtual-network.md)
### [Utiliser des VM nécessitant beaucoup de ressources système](batch-pool-compute-intensive-sizes.md)
### [Utiliser des nœuds de calcul Linux](batch-linux-nodes.md)
### [Utiliser des machines virtuelles de faible priorité](batch-low-pri-vms.md)

## Gérer des travaux et des tâches
### [Tâches d’achèvement et de préparation des travaux](batch-job-prep-release.md)
### [Tâches de nœud simultanées](batch-parallel-node-tasks.md)
### [Nombre des tâches par état](batch-get-task-counts.md)
### [Dépendances de la tâche](batch-task-dependencies.md)
### [Comptes d’utilisateur pour l’exécution des tâches](batch-user-accounts.md)
### [Exécuter des travaux Batch sans code](batch-cli-templates.md)


## [Conserver les résultats des tâches et des travaux](batch-task-output.md)
### [Conserver la sortie avec l’API Batch](batch-task-output-files.md)
### [Conserver la sortie avec la bibliothèque de conventions de fichier](batch-task-output-file-conventions.md)

## Exécuter des charges de travail
### [Travaux MPI](batch-mpi.md)
### [Charges de travail de conteneur](batch-docker-container-workloads.md)
### [Rendu](batch-rendering-service.md)
### [Traitement des données avec Batch et Data Factory](../data-factory/v1/data-factory-data-processing-using-batch.md?toc=%2fazure%2fbatch%2ftoc.json)

## Surveiller les ressources
### [Journalisation de diagnostic](batch-diagnostics.md)
### [Requêtes de liste efficaces](batch-efficient-list-queries.md)

## Utiliser des outils de développement
### [Utiliser Batch .NET](batch-dotnet-get-started.md)
### [Utiliser le code Python Batch](batch-python-tutorial.md)
### [Utiliser le fichier Node.js Batch](batch-nodejs-get-started.md)
### [Utiliser des cmdlets PowerShell](batch-powershell-cmdlets-get-started.md)
### [Utiliser l’interface de ligne de commande Microsoft Azure](batch-cli-get-started.md)

# Informations de référence

## [interface de ligne de commande Azure](/cli/azure/batch)
## [Azure PowerShell](/powershell/module/azurerm.batch)
## [.NET](/dotnet/api/overview/azure/batch)
## [Java](/java/api/overview/azure/batch)
## [Node.JS](/javascript/api/overview/azure/batch)
## [Python](/python/api/overview/azure/batch)
## REST
### [Service Batch](/rest/api/batchservice)
### [Gestion de Batch](/rest/api/batchmanagement/)
## [Analyse en mode batch](batch-analytics.md)
### [Événement de création de pool](batch-pool-create-event.md)
### [Événement de démarrage de suppression de pool](batch-pool-delete-start-event.md)
### [Événement de fin de suppression de pool](batch-pool-delete-complete-event.md)
### [Événement de démarrage de redimensionnement de pool](batch-pool-resize-start-event.md)
### [Événements de fin de redimensionnement de pool](batch-pool-resize-complete-event.md)
### [Événement de lancement de tâche](batch-task-start-event.md)
### [Événement de fin de tâche](batch-task-complete-event.md)
### [Événement d’échec de tâches](batch-task-fail-event.md)
## [Variables d’environnement de nœud de calcul Azure Batch](batch-compute-node-environment-variables.md)

# Ressources
## [Feuille de route Azure](https://azure.microsoft.com/roadmap/)
## [Forum Azure Batch](https://social.msdn.microsoft.com/Forums/en-us/home?forum=azurebatch)
## [Lot chantier](https://github.com/Azure/batch-shipyard)
## [Blog](https://blogs.technet.microsoft.com/windowshpc/)
## [Solutions HPC dans Azure](../virtual-machines/linux/high-performance-computing.md?toc=%2fazure%2fbatch%2ftoc.json)
## [Tarification](https://azure.microsoft.com/pricing/details/batch/)
## [Calculatrice de prix](https://azure.microsoft.com/pricing/calculator/)
## [modèle Azure Resource Manager](/azure/templates/microsoft.batch/batchaccounts)
## [Mises à jour de service](https://azure.microsoft.com/updates/?product=batch&updatetype=&platform=)
## [Dépassement de capacité de la pile](http://stackoverflow.com/questions/tagged/azure-batch)
## [Vidéos](https://azure.microsoft.com/documentation/videos/index/?services=batch)


