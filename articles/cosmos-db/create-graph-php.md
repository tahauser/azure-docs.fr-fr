...
Pour être en mesure de créer une base de données de graphes, vous devez avoir préalablement créé un compte de base de données Gremlin (Graph) avec Azure Cosmos DB.

[!INCLUDE [cosmos-db-create-dbaccount-graph](../../includes/cosmos-db-create-dbaccount-graph.md)]

## <a name="add-a-graph"></a>Ajout d’un graphe

Vous pouvez désormais utiliser l’outil Explorateur de données dans le portail Azure pour créer une base de données de graphes. 

1. Cliquez sur **Explorateur de données** > **Nouveau graphe**.

    La zone **Ajouter un graphe** est affichée à l’extrême droite. Il peut donc être nécessaire de faire défiler à droite pour l’afficher.

    ![Explorateur de données du portail Azure, page Ajouter un graphe](./media/create-graph-php/azure-cosmosdb-data-explorer-graph.png)

2. Dans la page **Ajouter un graphe**, entrez les paramètres relatifs au nouveau graphe.
    ...
    ID du graphe|sample-graph|Entrez le nom *sample-graph* pour votre nouvelle collection. Les noms de graphe sont soumis aux mêmes exigences de caractères que les ID de bases de données.
...
3. Exécutez la commande suivante pour cloner l’exemple de dépôt : Cette commande crée une copie de l’exemple d’application sur votre ordinateur. 
...
3. Remplacez le paramètre `username` dans l’objet de connexion par le nom de votre base de données et de votre graphe. Si vous avez utilisé les valeurs recommandées `sample-database` et `sample-graph`, vous devriez avoir ceci :
...
3. Dans la fenêtre de terminal git, utilisez la commande suivante pour démarrer l’application PHP.
    ...
    La fenêtre de terminal affiche les sommets ajoutés au graphe. 
...
Vous pouvez à présent revenir à l’Explorateur de données et voir les sommets ajoutés au graphe, ainsi qu’ajouter des points de données supplémentaires.

1. Cliquez sur **Explorateur de données**, développez **sample-graph**, cliquez sur **Graphe**, puis cliquez sur **Appliquer un filtre**. 
...
2. Dans la liste **Résultats**, observez les nouveaux utilisateurs ajoutés au graphe. Sélectionnez **ben** et notez qu’il est connecté à robin. Vous pouvez glisser-déplacer les sommets, effectuer un zoom avant et arrière à l’aide de la roulette de la souris et augmenter la taille du graphe avec la double flèche. 

   ![Nouveaux sommets dans le graphe de l’Explorateur de données du portail Azure](./media/create-graph-php/azure-cosmosdb-graph-explorer-new.png)

3. Nous allons à présent ajouter quelques nouveaux utilisateurs. Cliquez sur le bouton **Nouveau sommet** pour ajouter des données à votre graphe.
...
5. Cliquez sur **Ajouter une propriété** pour ajouter chacune des propriétés suivantes. Notez que vous pouvez créer des propriétés uniques pour chaque personne de votre graphe. Seule la clé id est obligatoire.
...
7. Cliquez de nouveau sur **Nouveau sommet** et ajoutez un nouvel utilisateur supplémentaire. 
...
11. Cliquez sur le bouton **Appliquer un filtre** avec le filtre par défaut `g.V()` pour afficher toutes les valeurs dans le graphe. Tous les utilisateurs apparaissent désormais dans la liste **Résultats**. 

    À mesure que vous ajoutez d’autres données, vous pouvez utiliser des filtres pour limiter les résultats renvoyés. Par défaut, l’Explorateur de données utilise `g.V()` pour récupérer tous les sommets d’un graphe. Vous pouvez choisir une autre [requête de graphe](tutorial-query-graph.md) comme `g.V().count()` pour retourner le nombre total de sommets du graphe au format JSON. Si vous avez modifié le filtre, choisissez de nouveau le filtre `g.V()` et cliquez sur **Appliquer un filtre** pour afficher de nouveau l’ensemble des résultats.
...
12. À présent, nous pouvons connecter rakesh et ashley. Vérifiez que **ashley** est bien sélectionné dans la liste **Résultats**, puis cliquez sur le bouton de modification en regard de **Cibles** dans la partie inférieure droite de l’écran. Vous devrez peut-être élargir la fenêtre pour visualiser la zone **Propriétés**.
   ...
   ![Modifier la cible d’un sommet dans un graphe](./media/create-graph-php/azure-cosmosdb-data-explorer-edit-target.png)
...
14. À présent, sélectionnez **rakesh** dans la liste des résultats et vérifiez qu’ashley et rakesh sont connectés. 
   ...
   ![Deux sommets connectés dans l’Explorateur de données](./media/create-graph-php/azure-cosmosdb-graph-explorer.png)

   La partie de ce didacticiel consacrée à la création de ressources est terminée. Vous pouvez continuer à ajouter des sommets dans votre graphe, modifier les sommets existants ou modifier les requêtes. Nous allons à présent examiner les métriques fournies par Azure Cosmos DB et nettoyer les ressources. 
...
Dans ce guide de démarrage rapide, vous avez appris à créer un compte Azure Cosmos D, à créer un graphe à l’aide de l’Explorateur de données et à exécuter une application. Vous pouvez maintenant générer des requêtes plus complexes et implémenter une logique de parcours de graphe puissante avec Gremlin. 
