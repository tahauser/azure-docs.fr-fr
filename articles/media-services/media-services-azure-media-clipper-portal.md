...
Pour créer un filtre de manifeste dynamique, sélectionnez le filtre de manifeste dynamique comme mode de découpage dans le menu des paramètres avancés. Vous pouvez suivre le processus de production d’un clip pour créer le filtre. Les filtres ne peuvent être produits que par rapport à un seul élément multimédia.
