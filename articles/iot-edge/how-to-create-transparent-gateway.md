...
L’article [Utilisation d’un appareil IoT Edge en tant que passerelle][lnk-edge-as-gateway] donne un aperçu conceptuel de la façon dont un appareil IoT Edge peut être utilisé en tant que passerelle transparente ou effectuer la translation de protocoles ou d’identités. Cet article fournit des instructions détaillées pour utiliser un appareil IoT Edge en tant que passerelle transparente. Dans le cadre de cet article, on entend par *passerelle IoT Edge* un appareil IoT Edge utilisé en tant que passerelle transparente.
...
1. Suivez l’étape 1 de l’[Exemple de gestion du certificat d’autorité de certification][lnk-ca-scripts] pour installer les scripts. Cloner à partir de la branche `modules-preview` :
...
* Dans le répertoire `certs`, exécutez `cat ./new-edge-device.cert.pem ./azure-iot-test-only.intermediate.cert.pem ./azure-iot-test-only.root.ca.cert.pem > ./new-edge-device-full-chain.cert.pem` pour obtenir la chaîne complète de la clé publique de l’appareil.
...
Par exemple, pour les applications .NET, vous pouvez ajouter l’extrait de code suivant afin d’approuver un certificat au format PEM stocké dans le chemin d’accès `certPath`. Si vous utilisez le script ci-dessus, le chemin d’accès fait référence à `certs/azure-iot-test-only.root.ca.cert.pem` (Bash) ou `RootCA.pem` (Powershell).
