...
2. Faites défiler jusqu'au bas de la liste. Vous pouvez cliquer avec le bouton droit sur **Write-Output** et sélectionner **Ajouter au canevas** ou cliquer sur les points de suspension en regard de la cmdlet, puis sélectionner **Ajouter au canevas**.
3. Cliquez sur l’activité **Write-Output** sur le canevas. Cette action ouvre le panneau du contrôle Configuration qui vous permet de configurer l’activité s’ouvre.
...
6. Sélectionnez le paramètre **InputObject** . Il s’agit du paramètre où vous spécifiez le texte à envoyer au flux de sortie.
7. Dans le menu déroulant **Source de données**, sélectionnez **Expression PowerShell**. Le menu déroulant **Source de données** fournit différentes sources que vous utilisez pour renseigner un paramètre.
   ...
   Vous pouvez utiliser la sortie de ces sources, par exemple une autre activité, un élément multimédia Automation ou une expression PowerShell. Dans ce cas, la sortie est simplement *Hello World*. Vous pouvez utiliser une expression PowerShell et spécifier une chaîne.
...
4. Lorsque la tâche du Runbook est terminée, sa sortie s'affiche. Dans ce cas, vous voyez *Hello World*.<br> ![Hello World](media/automation-first-runbook-graphical/runbook-test-results.png)
...
4. Démarrez le runbook en cliquant sur **Démarrer**, puis sur **Oui** quand vous y êtes invité.<br> ![Démarrer un Runbook](media/automation-first-runbook-graphical/runbook-controls-start-revised20165.png)
5. Un panneau de travail est ouvert pour le travail du runbook qui a été créé. Vérifiez que **État du travail** est défini sur **Terminé**.
7. Lorsque le Runbook prend l’état *Terminé*, cliquez sur **Sortie**. Le panneau **Sortie** est ouvert et vous pouvez voir le message *Hello World* affiché dans le volet.
8. Fermez le panneau Sortie.
9. Cliquez sur **Tous les journaux** pour ouvrir le panneau Flux de la tâche du Runbook. Vous devez uniquement voir le message *Hello World* dans le flux de sortie, mais d’autres flux peuvent s’afficher pour un travail de runbook, notamment Mode détaillé et Erreur si le runbook consigne ces informations.
10. Fermez le panneau Tous les journaux et le panneau Tâche pour revenir au panneau MyFirstRunbook.
11. Pour afficher tous les travaux du runbook, fermez le panneau **Travail** et sélectionnez **Travaux** sous **Ressources**. Il répertorie toutes les tâches créées par ce Runbook. Vous devez voir un seul travail, car vous n’avez exécuté le travail qu’une seule fois.
...
1. Dans le panneau Comptes Automation, sélectionnez **Variables** sous **RESSOURCES PARTAGÉES**.
2. Cliquez sur **Ajouter une variable**.<br>![Variable Automation](media/automation-first-runbook-graphical/create-new-subscriptionid-variable.png)
...
1. Revenez à votre runbook, puis sélectionnez **Modifier** dans le panneau MyFirstRunbook.<br> ![Modifier un Runbook](media/automation-first-runbook-graphical/runbook-controls-edit-revised20165.png)
2. Étant donné que **Write Hello World to output** ne vous est plus utile, cliquez sur les points de suspension et sélectionnez **Supprimer**.
3. Dans la bibliothèque, développez **RESSOURCES**, **Connexions** et ajoutez **AzureRunAsConnection** au canevas en sélectionnant **Ajouter au canevas**.
...
10. **Add-AzureRmAccount** comporte plusieurs ensembles de paramètres et vous devez donc en sélectionner un pour fournir les valeurs de paramètre. Cliquez sur **Jeu de paramètres**, puis sélectionnez le jeu de paramètres **ServicePrincipalCertificate**.
...
20. **Set-AzureRmContext** comporte plusieurs ensembles de paramètres et vous devez donc en sélectionner un pour fournir les valeurs de paramètre. Cliquez sur **Jeu de paramètres**, puis sélectionnez le jeu de paramètres **SubscriptionId**.  
...
5. Sélectionnez **Name**. Sélectionnez **Expression PowerShell** pour **Source de données** et tapez entre guillemets doubles le nom de la machine virtuelle que vous démarrez avec ce runbook. Cliquez sur **OK**.<br>![Valeur du paramètre Name Start-AzureRmVM](media/automation-first-runbook-graphical/runbook-startvm-nameparameter.png)
...
5. Sélectionnez **Get-AzureRmVM** puis **Jeu de paramètres** afin d’afficher les jeux pour **Get-AzureRmVM**. Sélectionnez le jeu de paramètres **GetVirtualMachineInResourceGroupNameParamSet** . Des points d’exclamation apparaissent en regard des paramètres **ResourceGroupName** et **Name**. Ils indiquent que ces paramètres sont obligatoires. Notez également que les deux attendent des valeurs de chaîne.
