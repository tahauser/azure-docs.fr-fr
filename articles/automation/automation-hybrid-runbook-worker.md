...
1. Téléchargez le script *New-OnPremiseHybridWorker.ps1* à partir de [PowerShell Gallery](https://www.powershellgallery.com/packages/New-OnPremiseHybridWorker/DisplayScript) directement depuis l’ordinateur qui exécute le rôle Runbook Worker hybride ou depuis un autre ordinateur dans votre environnement, et copiez-le dans le Worker.
   ...
   * *OMSResourceGroupName* (facultatif) : le nom du groupe de ressources pour l’espace de travail OMS. S’il n’est pas spécifié, le AAResourceGroupName est utilisé.
   * *HybridGroupName* (obligatoire) : le nom d’un groupe Runbook Worker hybride que vous spécifiez comme cible pour les runbooks prenant en charge ce scénario.
   ...
   * *WorkspaceName* (facultatif) : le nom de l’espace de travail Log Analytics. Si vous n’avez pas d’espace de travail Log Analytics, le script en crée un et le configure.
     ...
     > [!NOTE]
     > Actuellement, les seules régions Automation prises en charge pour l’intégration à Log Analytics sont : **Sud-Est de l’Australie**, **Est des États-Unis 2**, **Sud-Est asiatique** et **Europe de l’Ouest**. Si votre compte Automation ne se trouve pas dans une de ces régions, le script crée un espace de travail Log Analytics, mais vous avertit qu’il ne peut pas lier les deux.

2. Sur votre ordinateur, démarrez **Windows PowerShell** à partir de l’écran **Démarrer** en mode Administrateur.
3. Dans l’interpréteur de lignes de commandes PowerShell, accédez au dossier qui contient le script que vous avez téléchargé, puis exécutez-le en modifiant les valeurs par des paramètres *-AutomationAccountName*, *-ResourceGroupName*, *-HybridGroupName*, *-SubscriptionId* et *-SubscriptionId* et *-WorkspaceName*.
...
4. Vous êtes invité à accepter d’installer **NuGet** et à vous authentifier avec vos informations d’identification Azure.
...
Microsoft Monitoring Agent connecte les ordinateurs à Operations Management Suite. Lorsque vous installez l’agent sur votre ordinateur local et que vous le connectez à votre espace de travail, il télécharge les composants requis pour le Runbook Worker hybride.
...
Pour importer le module, ouvrez une session PowerShell en mode administrateur et exécutez les commandes suivantes :
...
1. Sous **Automatisation de processus**, sélectionnez **Groupes de Workers hybrides**. Sélectionnez le groupe que vous souhaitez supprimer. Une fois que vous avez sélectionné le groupe, le panneau de propriétés du **groupe de Workers hybrides** s’affiche.

   ![Panneau du groupe Runbook Worker hybride](media/automation-hybrid-runbook-worker/automation-hybrid-runbook-worker-group-properties.png)

1. Dans le panneau des propriétés du groupe sélectionné, cliquez sur **Supprimer**. Un message apparaît, vous invitant à confirmer cette action : sélectionnez **Oui** si vous êtes sûr de vouloir continuer.
